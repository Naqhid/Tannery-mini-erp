...
export default function ProductionWipReport({ embedded }: { embedded?: boolean }) {
  ...
  const columns: Column<Row>[] = [
    ...
    { key: 'wip_qty', header: 'WIP (In − Out − Rej)', align: 'right', render: r => <span className={Number(r.wip_qty) < 0 ? 'font-semibold text-red-600' : 'font-semibold text-amber-700'}>{fmtQty(r.wip_qty)}</span> },
  ];
