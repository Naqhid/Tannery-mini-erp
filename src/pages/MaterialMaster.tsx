...
const fmtQty = (n: any) => new Intl.NumberFormat('en-IN', { maximumFractionDigits: 2 }).format(Number(n) || 0);
const fmtValue = (n: any) => new Intl.NumberFormat('en-IN', { style: 'currency', currency: 'INR', maximumFractionDigits: 2 }).format(Number(n) || 0);
...
export default function MaterialMaster() {
  ...
  const columns = [
    ...
    { key: 'group_name', header: 'Group', sortable: true, render: (row: any) => <span className="text-gray-700">{row.group_name || '—'}</span> },
    ...
    { key: 'opening_stock', header: 'Opening Stock Qty', sortable: true, render: (row: any) => <span className="text-gray-800 tabular-nums block text-center">{fmtQty(row.opening_stock)}</span> },
    { key: 'opening_stock_value', header: 'Opening Stock Value', sortable: true, render: (row: any) => <span className="text-gray-800 tabular-nums block text-center">{fmtValue(row.opening_stock_value)}</span> },
  ...
  const exportColumns = [
    ...
    { key: 'opening_stock', header: 'Opening Stock Qty' },
    { key: 'opening_stock_value', header: 'Opening Stock Value' },
