...
-- Fixed version for MySQL (removed IF NOT EXISTS from ADD COLUMN)
-- ============================================================
...
-- Note: We need to check if columns exist first (MySQL doesn't support IF NOT EXISTS for ADD COLUMN)

-- First, check and add columns one by one using stored procedures or manual checks
-- For simplicity, we'll just add them without IF NOT EXISTS and handle errors

ALTER TABLE supplier_pricing
  ADD COLUMN item_group VARCHAR(100) AFTER material_id,
  ADD COLUMN supplier_part_no VARCHAR(100) AFTER item_group,
  ADD COLUMN unit_price DECIMAL(12,2) DEFAULT 0 AFTER uom,
  ADD COLUMN currency VARCHAR(10) DEFAULT 'INR' AFTER unit_price,
  ADD COLUMN min_order_qty DECIMAL(12,2) DEFAULT 0 AFTER currency,
  ADD COLUMN price_type ENUM('Purchase Price','Contract Price') DEFAULT 'Purchase Price' AFTER min_order_qty,
  ADD COLUMN remarks TEXT AFTER status,
  ADD COLUMN approved_by INT AFTER remarks,
  ADD COLUMN approved_date DATE AFTER approved_by,
  ADD COLUMN approval_notes TEXT AFTER approved_date,
  ADD COLUMN last_approved_price DECIMAL(12,2) DEFAULT 0 AFTER approval_notes,
  ADD COLUMN last_approved_date DATE AFTER last_approved_price,
...
-- Using stored procedure to check if constraint exists
DELIMITER $$
CREATE PROCEDURE add_foreign_key_if_not_exists(
    IN table_name VARCHAR(64),
    IN constraint_name VARCHAR(64),
    IN constraint_sql VARCHAR(512)
)
BEGIN
...
-- Add foreign keys
CALL add_foreign_key_if_not_exists('supplier_pricing', 'fk_sp_approved_by', 'ALTER TABLE supplier_pricing ADD CONSTRAINT fk_sp_approved_by FOREIGN KEY (approved_by) REFERENCES users(id) ON DELETE SET NULL');
CALL add_foreign_key_if_not_exists('supplier_pricing', 'fk_sp_created_by', 'ALTER TABLE supplier_pricing ADD CONSTRAINT fk_sp_created_by FOREIGN KEY (created_by) REFERENCES users(id) ON DELETE SET NULL');
CALL add_foreign_key_if_not_exists('supplier_pricing', 'fk_sp_updated_by', 'ALTER TABLE supplier_pricing ADD CONSTRAINT fk_sp_updated_by FOREIGN KEY (updated_by) REFERENCES users(id) ON DELETE SET NULL');

-- Add indexes for better performance
ALTER TABLE supplier_pricing
  ADD INDEX idx_sp_status (status),
  ADD INDEX idx_sp_valid_from (valid_from),
  ADD INDEX idx_sp_valid_to (valid_to),
  ADD INDEX idx_sp_deleted (deleted_at);
...
-- Using simple ALTER TABLE since MySQL doesn't support IF NOT EXISTS
