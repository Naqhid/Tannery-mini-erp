...
export async function softDelete(id) {
  const [result] = await pool.query(
    `UPDATE suppliers SET deleted_at = CURRENT_TIMESTAMP, status = 'Inactive' WHERE id = ? AND deleted_at IS NULL`,
    [id]
  );
  return result.affectedRows > 0;
}
...
export async function restore(id) {
  const [result] = await pool.query(
    `UPDATE suppliers SET deleted_at = NULL, status = 'Active' WHERE id = ?`,
    [id]
  );
  return result.affectedRows > 0;
}
...
export async function bulkSoftDelete(ids) {
  ...
  const [result] = await pool.query(
    `UPDATE suppliers SET deleted_at = CURRENT_TIMESTAMP, status = 'Inactive' WHERE id IN (${placeholders}) AND deleted_at IS NULL`,
    ids
  );
...
export async function bulkUpdateStatus(ids, status) {
  ...
  const placeholders = ids.map(() => '?').join(', ');
  const [result] = await pool.query(
    `UPDATE suppliers SET status = ? WHERE id IN (${placeholders}) AND deleted_at IS NULL`,
    [status, ...ids]
  );
  return result.affectedRows;
}
...
export async function checkDuplicate(data, excludeId = null) {
  ...
  let query = `SELECT id, code, name FROM suppliers WHERE name = ? AND deleted_at IS NULL`;
...
export async function getAuditInfo(id) {
  const [rows] = await pool.query(
    `SELECT id, code, name, created_by, created_at, updated_by, updated_at, deleted_at FROM suppliers WHERE id = ?`,
    [id]
  );
  return rows[0] || null;
}
...
export async function dropdown() {
  const [rows] = await pool.query(
    `SELECT id, code, name FROM suppliers WHERE status = 'Active' AND deleted_at IS NULL ORDER BY name ASC`
  );
  return rows;
}
