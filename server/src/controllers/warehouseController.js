...
export async function listBins(req, res, next) {
  try {
    const rows = await model.getBins(req.params.id);
    res.json({ data: rows });
  } catch (err) { next(err); }
}

export async function saveBins(req, res, next) {
  try {
    const createdBy = req.user?.id || null;
    const rows = await model.saveBins(req.params.id, req.body.bins || [], createdBy);
    res.json({ data: rows, message: 'Bins saved successfully!' });
  } catch (err) { next(err); }
}

export async function deleteBin(req, res, next) {
  try {
    const ok = await model.deleteBin(req.params.binId);
...
export async function listUserAccess(req, res, next) {
  try {
    const rows = await model.getUserAccess(req.params.id);
    res.json({ data: rows });
  } catch (err) { next(err); }
}

export async function saveUserAccess(req, res, next) {
  try {
    const createdBy = req.user?.id || null;
    const rows = await model.saveUserAccess(req.params.id, req.body.users || [], createdBy);
    res.json({ data: rows, message: 'User access saved successfully!' });
  } catch (err) { next(err); }
}

export async function deleteUserAccess(req, res, next) {
  try {
    const ok = await model.deleteUserAccess(req.params.accessId);
    if (!ok) return res.status(404).json({ error: 'User access entry not found' });
