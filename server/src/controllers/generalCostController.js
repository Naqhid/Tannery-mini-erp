import * as model from '../models/generalCostModel.js';
...
export async function getOne(req, res, next) {
  try {
    ...
    if (!entry) return res.status(404).json({ error: 'General Cost entry not found' });
    res.json({ data: entry });
  } catch (err) { next(err); }
}

export async function getByPlan(req, res, next) {
  try {
    const entry = await model.getByPlanId(req.params.planId);
    if (!entry) return res.status(404).json({ error: 'No general cost entry for this plan' });
    res.json({ data: entry });
  } catch (err) { next(err); }
}
...
export async function create(req, res, next) {
  try {
    ...
    res.status(201).json({ data: result, message: 'General Cost entry created successfully!' });
  } catch (err) { next(err); }
}
...
export async function update(req, res, next) {
  try {
    ...
    res.json({ data: result, message: 'General Cost entry updated successfully!' });
  } catch (err) {
...
export async function post(req, res, next) {
  try {
    ...
    if (!ok) return res.status(404).json({ error: 'General Cost entry not found' });
    res.json({ data: { id: req.params.id, status: 'Posted' }, message: 'General Cost posted successfully!' });
  } catch (err) {
...
export async function remove(req, res, next) {
  try {
    ...
    if (!ok) return res.status(404).json({ error: 'General Cost entry not found' });
    res.json({ data: { id: req.params.id, deleted: true }, message: 'General Cost entry deleted successfully!' });
  } catch (err) {
