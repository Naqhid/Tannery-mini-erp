...
import { Save, X, ArrowLeft, Plus, Trash2, Truck } from 'lucide-react';
...
const emptyItem: Item = { _key: '', material_id: '', uom: '', order_qty: '', received_qty: '', rate: '', amount: 0, batch_no: '', expiry_date: '' };
...
export default function MaterialReceiptEntryDetail() {
  ...
  const grandTotal = totalAmount + freight + loadingCharges + otherCharges;
    ...
    <div className="space-y-5">
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          <button onClick={() => navigate('/material-receipt')} className="inline-flex items-center gap-2 px-4 py-2.5 text-xs font-bold text-gray-600 bg-white border-2 border-gray-200 rounded-xl hover:bg-gray-50 transition-all">
            <X size={14} /> Cancel
          </button>
          <button onClick={handleSave} disabled={saving} className="inline-flex items-center gap-2 px-5 py-2.5 text-xs font-bold text-white bg-gradient-to-r from-orange-600 to-amber-600 rounded-xl shadow-lg hover:shadow-xl transition-all disabled:opacity-50">
            <Save size={14} /> {saving ? 'Saving...' : 'Save Receipt'}
          </button>
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-gray-200 shadow-lg p-6">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          <Input label="Receipt No" value={receipt.receipt_no} onChange={(e) => update('receipt_no', e.target.value)} placeholder="Auto-generated" />
          ...
          <Select label="Warehouse" required options={[{ value: '', label: 'Select warehouse' }, ...warehouses.map((w) => ({ value: String(w.id), label: `${w.name} (${w.code})` }))]} value={receipt.warehouse_id} onChange={(e) => update('warehouse_id', e.target.value)} />
          <Select label="Supplier" options={[{ value: '', label: 'Select supplier' }, ...suppliers.map((s) => ({ value: String(s.id), label: `${s.name} (${s.code})` }))]} value={receipt.supplier_id} onChange={(e) => update('supplier_id', e.target.value)} />
          <Input label="Purchase Order No" value={receipt.purchase_order_no} onChange={(e) => update('purchase_order_no', e.target.value)} placeholder="PO number" />
          <Input label="PO Date" type="date" value={receipt.po_date} onChange={(e) => update('po_date', e.target.value)} />
          <Input label="Gate Entry No" value={receipt.gate_entry_no} onChange={(e) => update('gate_entry_no', e.target.value)} placeholder="Gate entry" />
          <Input label="Challan No" value={receipt.challan_no} onChange={(e) => update('challan_no', e.target.value)} placeholder="Challan number" />
          <Input label="Challan Date" type="date" value={receipt.challan_date} onChange={(e) => update('challan_date', e.target.value)} />
          <Input label="LR/GRN No" value={receipt.lr_grn_no} onChange={(e) => update('lr_grn_no', e.target.value)} placeholder="LR/GRN number" />
          <Input label="LR/GRN Date" type="date" value={receipt.lr_grn_date} onChange={(e) => update('lr_grn_date', e.target.value)} />
          <Input label="Transporter" value={receipt.transporter} onChange={(e) => update('transporter', e.target.value)} placeholder="Transporter name" />
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl border border-gray-200 shadow-lg overflow-hidden">
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 bg-gradient-to-r from-slate-50 to-orange-50/30">
          <h2 className="text-sm font-bold text-gray-900 uppercase tracking-wide">Received Items</h2>
          <button onClick={addItem} className="inline-flex items-center gap-2 px-3 py-2 text-xs font-bold text-orange-600 bg-orange-50 border border-orange-200 rounded-xl hover:bg-orange-100 transition-all">
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="bg-slate-50 border-b border-gray-200">
                ...
                <th className="text-left py-3 px-4 text-[11px] font-bold text-gray-600 uppercase">Order Qty</th>
                <th className="text-left py-3 px-4 text-[11px] font-bold text-gray-600 uppercase">Received Qty</th>
                <th className="text-left py-3 px-4 text-[11px] font-bold text-gray-600 uppercase">Rate</th>
                ...
                <th className="text-left py-3 px-4 text-[11px] font-bold text-gray-600 uppercase">Expiry</th>
                ...
                <tr key={item._key} className="hover:bg-orange-50/30 transition-all">
                  ...
                  <td className="py-2 px-4">
                    <select value={item.material_id} onChange={(e) => updateItem(item._key, 'material_id', e.target.value)}
                      className="w-full px-2 py-1.5 text-xs border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-orange-500/20 focus:border-orange-500 bg-white min-w-[180px]">
                  ...
                  <td className="py-2 px-4">
                    <input value={item.uom} onChange={(e) => updateItem(item._key, 'uom', e.target.value)}
                      className="w-full px-2 py-1.5 text-xs border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-orange-500/20 focus:border-orange-500 min-w-[80px]" />
                  </td>
                  <td className="py-2 px-4">
                    <input type="number" value={item.order_qty} onChange={(e) => updateItem(item._key, 'order_qty', e.target.value)}
                      className="w-full px-2 py-1.5 text-xs border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-orange-500/20 focus:border-orange-500 min-w-[90px]" />
                  </td>
                  <td className="py-2 px-4">
                  ...
                  <td className="py-2 px-4">
                    <input type="number" value={item.rate} onChange={(e) => updateItem(item._key, 'rate', e.target.value)}
                      className="w-full px-2 py-1.5 text-xs border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-orange-500/20 focus:border-orange-500 min-w-[100px]" />
                  </td>
                  <td className="py-2 px-4 font-bold text-orange-700 text-xs">₹{(item.amount || 0).toLocaleString('en-IN', { minimumFractionDigits: 2 })}</td>
                  <td className="py-2 px-4">
            ...
            <tfoot>
              <tr className="bg-gradient-to-r from-orange-50 to-amber-50 border-t-2 border-orange-200">
                <td colSpan={6} className="py-3 px-4 text-right text-xs font-bold text-gray-700 uppercase">Sub Total:</td>
                <td className="py-3 px-4 text-lg font-black text-orange-700">₹{totalAmount.toLocaleString('en-IN', { minimumFractionDigits: 2 })}</td>
                <td colSpan={3}></td>
              </tr>
            </tfoot>
          </table>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        <div className="bg-white rounded-2xl border border-gray-200 shadow-lg p-6 lg:col-span-1">
          ...
          <div className="space-y-3">
            <Input label="Freight" type="number" value={receipt.freight} onChange={(e) => update('freight', e.target.value)} placeholder="0.00" />
            <Input label="Loading Charges" type="number" value={receipt.loading_charges} onChange={(e) => update('loading_charges', e.target.value)} placeholder="0.00" />
            <Input label="Other Charges" type="number" value={receipt.other_charges} onChange={(e) => update('other_charges', e.target.value)} placeholder="0.00" />
            <div className="flex items-center justify-between pt-3 border-t border-gray-100">
              <span className="text-sm font-bold text-gray-900">Grand Total:</span>
              <span className="text-xl font-black text-orange-700">₹{grandTotal.toLocaleString('en-IN', { minimumFractionDigits: 2 })}</span>
            </div>
          </div>
        </div>
        <div className="bg-white rounded-2xl border border-gray-200 shadow-lg p-6 lg:col-span-2">
          ...
          <textarea rows={6} value={receipt.remarks} onChange={(e) => update('remarks', e.target.value)} placeholder="Additional remarks..."
            className="w-full px-3 py-2 text-xs text-gray-900 border border-gray-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-orange-500/20 focus:border-orange-500 transition-all resize-none" />
        </div>
      </div>
    </div>
  );
